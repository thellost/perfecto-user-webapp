...
async function PropertyDetails({params} : {
    ...
    const handleSaveProperty = async() => {
        ...
        try {
            ...
            (reponse. status === 200) {toast. success("Property saved successfully to wishlist");fetchPropertyDetails();
            } else
            { toast. error("Failed to save property");
            }
        } catch
        (error) {console. error("Error saving property:", error);toast. error(error. response. data. detail || "Failed to save property");
        }
    };

    const
    handleRemoveProperty = async() => { try
        { const
            reponse = await axios. post( `${process. env. REACT_APP_API_URL}/wishlist/remove/${_id}`, {}, { // Empty payload
                    headers:
                { Authorization:
                    `Bearer ${access_token}`
                }
            });
            if (reponse.status ===200) { toast.
                success("Property removed successfully from wishlist"); fetchPropertyDetails();
            } else
                {
            toast.
            ...
            (error) { console.
            error("Error removing property:",error); toast.
            error(error.response.data.detail ||"Failed to remove property");
        }
    };

        const
    handleSaveLoanDetails = (loanDetails) => {console.
        log("Saved loan details:",loanDetails); setIsModalOpen(false);
    };
    ...
    toggleDescription = () => {setShowFullDescription(! showFullDescription);
        ...
        const
    truncateDescription = (text,wordLimit) => {const words = text. split(" "); if(words.length <=wordLimit) { returntext;
        }
        return
            words
            ?
        ...
        const
    wordLimit = 100; const displayDescription = showFullDescription
        ? description
        : truncateDescription(description,wordLimit);

    // Monthly Payment Calculationconst
    calculateMonthlyPayment = (principal,rate, term) => {const monthlyRate = rate / 100 / 12; const numPayments = term * 12; return ((principal *monthlyRate) / (1 -Math. pow(1 +monthlyRate, -numPayments))).toFixed(2);
    };

    // Calculating "Others Monthly Payment"
    const
    othersInterestRate = 7.25; //  interest rate for others const
    principal = price - downPayment; const othersMonthlyPayment = calculateMonthlyPayment(principal,othersInterestRate, terms);
    ...
    options = { responsive:
        ...
        { legend:
            ...
            { callbacks:
                { label:
                    function (tooltipItem) { letvalue = tooltipItem. raw;return `$${value. toLocaleString()}`;
                    }
                }
            }
        },
        ...
        { x:
            { beginAtZero:
                true, ticks:
                { callback:
                    function (value,index) { returndata. labels[index];
                    }
                }
            },
                y:
            ...
            <div className="col-span-3">
                ...
                <div className="sm:hidden block px-[24px]">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <Button className={"border-[2px] w-full"} placeholder="Share" variant="white">
                            <FiShare className="ml-2 text-[18px] text-[#f08e80] transform rotate-90"/>
                        </Button>
                    </div>
                </div>
