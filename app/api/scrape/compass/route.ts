        ...
        try {
            ...
            try {
                ...
                const infoElement = await page.evaluate(() => {
                  let name = document?.querySelector('.summary__StyledAddress-e4c4ok-8')?.innerHTML;
                  let urlElement = document?.querySelector('#media-gallery-hero-image');
                  let url = urlElement ? urlElement?.getAttribute('src') : null;
                  const price =
                    document?.querySelector('.summary__StyledSummaryDetailUnit-sc-e4c4ok-4')?.querySelector('.textIntent-title2')?.innerHTML || null;
                ...
                const imageUrls = result?.props?.listingRelation?.listing?.media?.map(
                  (img: { originalUrl: any; }) => img?.originalUrl,
                );
                ...
                const buildingInfo = await page.evaluate(() => {
                  ...
                  data.forEach((element) => {
                    ...
                    buildingInfoMap.set(toCamelCase(key ?? null), value);
                  });
                ...
                const homeFacts = await page.evaluate(() => {
                  ...
                  data.forEach((element) => {
                    ...
                    value = value ? value.replace(/[,\bSq. Ft.]/g, '') : '';
                ...
                const publicRecords = await page.evaluate(() => {
                  function toCamelCase(str: string | undefined) {
                    return str?.replace(/(?:^\w|[A-Z]|\b\w|\s+)/g, function (match, index) {
                      if (+match === 0) return '';
                      return index === 0 ? match.toLowerCase() : match.toUpperCase();
                    });
                ...
                const homeForSale = await page.evaluate(() => {
                  let categories: { name: string | undefined; links: any[]; }[] = [];
                ...
                for (let i = 0; i < ans1?.length; i++) {
                  ...
                  let records = {};
                  let records3 : { [key: string]: any } = {};
                  
                  for (let k = 0; k < ans1[i]?.subCategories.length; k++) {
                    let records2 : { [key: string]: any }= {};
