...
export async function GET(req: NextRequest) {
  try {
    ...
    const offerings = await getOfferingsByPropertyIds(propertyIdList);
    return NextResponse.json({
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    if( !offeringId && !propertyId) {
      return NextResponse.json({ error: 'Offering ID or Property ID is required' }, { status: 400 });
    }
    if (!offeringId || !propertyId) {
      return NextResponse.json({ error: 'Offering ID and Property ID must not be null' }, { status: 400 });
    }
